...
//		}
//		else {
//			listEmployee=listEmployee.stream().filter((var)->var.getEmpName())&&var.get
//		}
		
		if(searchEmployee.getBand()!='0'){
			listEmployee=listEmployee.stream().filter((var)->var.getEmpName()
				.startsWith(searchEmployee.getEmpName()) && var.getSalary() > searchEmployee
				.getSalary() &&var.getBand()==searchEmployee.getBand())
					.collect(Collectors.toList());
